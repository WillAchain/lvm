...
namespace lvm {
    namespace blockchain {
        ...
        StorageDataType StorageDataType::get_storage_data_from_lua_storage(const GluaStorageValue& lua_storage) {
            ...
            if (lua_storage.type == lvm::blockchain::StorageValueTypes::storage_value_null) {
                storage_data = StorageDataType(StorageNullType());
            } else if (lua_storage.type == lvm::blockchain::StorageValueTypes::storage_value_int) {
                storage_data = StorageDataType(StorageIntType(lua_storage.value.int_value));
            } else if (lua_storage.type == lvm::blockchain::StorageValueTypes::storage_value_number) {
                storage_data = StorageDataType(StorageNumberType(lua_storage.value.number_value));
            } else if (lua_storage.type == lvm::blockchain::StorageValueTypes::storage_value_bool) {
                storage_data = StorageDataType(StorageBoolType(lua_storage.value.bool_value));
            } else if (lua_storage.type == lvm::blockchain::StorageValueTypes::storage_value_string) {
                storage_data = StorageDataType(StorageStringType(std::string(lua_storage.value.string_value)));
            } else if (lvm::blockchain::is_any_array_storage_value_type(lua_storage.type)
                       || lvm::blockchain::is_any_table_storage_value_type(lua_storage.type)) {
                if (get_storage_base_type(lua_storage.type) == lvm::blockchain::StorageValueTypes::storage_value_int) {
                    ...
                    if (lua_storage.type == lvm::blockchain::StorageValueTypes::storage_value_int_table) {
                        storage_data = StorageDataType(StorageIntTableType(int_map));
                    } else if (lua_storage.type == lvm::blockchain::StorageValueTypes::storage_value_int_array) {
                        storage_data = StorageDataType(StorageIntArrayType(int_map));
                    }
                } else if (get_storage_base_type(lua_storage.type) == lvm::blockchain::StorageValueTypes::storage_value_number) {
                    std::map<std::string, double> number_map;
                    std::for_each(lua_storage.value.table_value->begin(), lua_storage.value.table_value->end(),
                    [&](const std::pair<std::string, struct GluaStorageValue>& item) {
                        number_map.insert(std::make_pair(item.first, item.second.value.number_value));
                    });
                    if (lua_storage.type == lvm::blockchain::StorageValueTypes::storage_value_number_table) {
                        storage_data = StorageDataType(StorageNumberTableType(number_map));
                    } else if (lua_storage.type == lvm::blockchain::StorageValueTypes::storage_value_number_array) {
                        storage_data = StorageDataType(StorageNumberArrayType(number_map));
                    }
                } else if (get_storage_base_type(lua_storage.type) == lvm::blockchain::StorageValueTypes::storage_value_bool) {
                    std::map<std::string, bool> bool_map;
                    std::for_each(lua_storage.value.table_value->begin(), lua_storage.value.table_value->end(),
                    [&](const std::pair<std::string, struct GluaStorageValue>& item) {
                        bool_map.insert(std::make_pair(item.first, item.second.value.bool_value));
                    });
                    if (lua_storage.type == lvm::blockchain::StorageValueTypes::storage_value_bool_table) {
                        storage_data = StorageDataType(StorageBoolTableType(bool_map));
                    } else if (lua_storage.type == lvm::blockchain::StorageValueTypes::storage_value_bool_array) {
                        storage_data = StorageDataType(StorageBoolArrayType(bool_map));
                    }
                } else if (get_storage_base_type(lua_storage.type) == lvm::blockchain::StorageValueTypes::storage_value_string) {
                    std::map<std::string, std::string> string_map;
                    std::for_each(lua_storage.value.table_value->begin(), lua_storage.value.table_value->end(),
                    [&](const std::pair<std::string, struct GluaStorageValue>& item) {
                        string_map.insert(std::make_pair(item.first, item.second.value.string_value));
                    });
                    if (lua_storage.type == lvm::blockchain::StorageValueTypes::storage_value_string_table) {
                        storage_data = StorageDataType(StorageStringTableType(string_map));
                    } else if (lua_storage.type == lvm::blockchain::StorageValueTypes::storage_value_string_array) {
        ...
        GluaStorageValue StorageDataType::create_lua_storage_from_storage_data(lua_State *L, const StorageDataType& storage) {
            ...
            if (storage.storage_type == StorageValueTypes::storage_value_null) {
                p_lua_storage->type = lvm::blockchain::StorageValueTypes::storage_value_null;
            } else if (storage.storage_type == StorageValueTypes::storage_value_int) {
                p_lua_storage->type = lvm::blockchain::StorageValueTypes::storage_value_int;
                ...
                p_lua_storage->type = lvm::blockchain::StorageValueTypes::storage_value_number;
                ...
                p_lua_storage->type = lvm::blockchain::StorageValueTypes::storage_value_bool;
                ...
                p_lua_storage->type = lvm::blockchain::StorageValueTypes::storage_value_string;
                std::string storage_string = storage.as<StorageStringType>().raw_storage;
                size_t string_len = storage_string.length();
                p_lua_storage->value.string_value = lvm::lua::lib::malloc_managed_string(L, string_len + 1);
                ...
                lvm::blockchain::is_any_array_storage_value_type(storage.storage_type)
                || lvm::blockchain::is_any_table_storage_value_type(storage.storage_type)
            ) {
                p_lua_storage->type = storage.storage_type;
                p_lua_storage->value.table_value = lvm::lua::lib::create_managed_lua_table_map(L);
                ...
                if (storage.storage_type == StorageValueTypes::storage_value_int_table ||
                    ...
                    for (const auto& item : int_table) {
                        ...
                        base_storage.type = lvm::blockchain::StorageValueTypes::storage_value_int;
                    ...
                    for (const auto& item : double_table) {
                        GluaStorageValue base_storage;
                        base_storage.type = lvm::blockchain::StorageValueTypes::storage_value_number;
                    ...
                    for (const auto& item : bool_table) {
                        GluaStorageValue base_storage;
                        base_storage.type = lvm::blockchain::StorageValueTypes::storage_value_bool;
                    ...
                    for (const auto& item : string_table) {
                        GluaStorageValue base_storage;
                        base_storage.type = lvm::blockchain::StorageValueTypes::storage_value_string;
                        size_t string_len = item.second.length();
                        base_storage.value.string_value = lvm::lua::lib::malloc_managed_string(L, string_len + 1);
